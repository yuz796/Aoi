...
public:
    ...
    ELet(var_list vars, std::shared_ptr<Expr> body) : Expr(ELET), vars(vars), body(body) {}
...
public:
    ...
    operator std::string() const override {
        ...
        switch (op) {
            case EPrim::PLUS:  ss << "PLUS"; break;
            case EPrim::MINUS: ss << "MINUS"; break;
            case EPrim::TIMES: ss << "TIMES"; break;
        }
...
int main() {
    ...
    auto v2 = EId("x");
    ELet::var_list vars{std::make_pair("x", &v1), std::make_pair("y", &v2)};
    auto body = EId("y");
    auto prog = ELet(vars, &body);
    
    auto result = compile_expr(&prog, 1, std::list<std::pair<std::string, int64_t>>());
    
    std::cout << "section .text\nglobal our_code_starts_here\nour_code_starts_here:" << std::endl;
