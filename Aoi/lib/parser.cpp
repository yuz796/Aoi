...
struct binding : pegtl::seq<list_begin, binding_symbol, whitespace, expr, pegtl::must<list_end>> {};

struct bindings_begin : pegtl::seq<pegtl::one<'('>, pegtl::star<pegtl::space>> {};
struct bindings_end : pegtl::one<')'> {};

struct bindings : pegtl::seq<bindings_begin, pegtl::list<binding, pegtl::plus<pegtl::space>>,
                             pegtl::must<bindings_end>> {};

struct let : pegtl::seq<list_begin, pegtl::string<'l', 'e', 't'>, whitespace, bindings, whitespace, expr,
                        pegtl::must<list_end>> {};

struct prim : pegtl::seq<list_begin, binop, whitespace, expr, whitespace, expr2, pegtl::must<list_end>> {};
...
struct action<binop> {
    ...
    static void apply(const ActionInput &in, parser_state &state) {
        switch (in.string().front()) {
            case '+': state.ops.push_back(IR::EPrim::PLUS);  break;
            case '-': state.ops.push_back(IR::EPrim::MINUS); break;
            case '*': state.ops.push_back(IR::EPrim::TIMES); break;
        }
    }
};
...
struct action<prim> {
    static void apply0(parser_state &state) {
        ...
        state.ops.pop_back();
        state.result.push_back(std::make_shared<IR::EPrim>(op, std::move(e1), std::move(e2)));
    }
};

template<>
struct action<binding_symbol> {
    template <typename ActionInput>
    static void apply(const ActionInput &in, parser_state &state) {
...
template<>
struct action<binding> {
    static void apply0(parser_state &state) {
        state.binding_lists.back().push_back(std::make_pair(std::move(state.vars.back()),
                                                           std::move(state.result.back())));
        state.vars.pop_back();
        state.result.pop_back();
    }
};
...
struct action<let> {
    static void apply0(parser_state &state) {
        ...
        state.result.push_back(std::make_shared<IR::ELet>(std::move(binding_list), std::move(e)));
    }
};
...
int main(int argc, char *argv[]) {
    ...
    /*using namespace IR;
    auto v1 = ENumber(37364);
    auto v2 = ENumber(1);
    auto v3 = EId("x");
    auto v4 = EPrim(EPrim::PLUS, &v3, &v2);
    ELet::var_list vars{std::make_pair("x", &v1), std::make_pair("y", &v4)};
    auto body = EId("y");
    auto prog = ELet(vars, &body);
